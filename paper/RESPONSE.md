...
## Second enrichment round (author-directed, 2026-08-13..15) — computed on a collaborator's machine

> **Numbering note.** `E1`–`E9` above are the *first*-round enrichments and their ids are fixed. The
> collaborator's own proposal notes used a separate `E1`–`E5` / `P1`–`P6` scheme for the same period;
...
> than read off the collaborator's summary.
