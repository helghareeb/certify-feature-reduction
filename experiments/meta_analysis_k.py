...
the data already in the canonical summary — no new model fits:
  ...
  1. the retained-k table per dataset x budget (the rule's consequences, incl. the k=1 collapse);
  2. rho of harm against p, k_min, and log2(k_min), plus the k=1-collapse indicator;
  3. rho(p, harm) EXCLUDING the two datasets whose ladder rounds to k=1 (haberman, mammographic);
...
def main() -> int:
    ...
    sub = df[~df["dataset"].isin(K1_DATASETS)]
