"""Tier 1 — materiality of the ECE bin-sweep (supervisor's follow-up).
...
pen_path = os.path.join(REPO,"results","tier1","ece_penalties.parquet")
...
res.to_csv(os.path.join(REPO,"results","tier1","tier1_ece_materiality.csv"),index=False)
...
print("\n=== the logistic sign flip — detail ===")
for bt in ("equal_width","equal_freq"):
    s=res[(res.classifier=='logistic')&(res.calibrate=='none')&(res.bin_type==bt)].sort_values("bins")
    signs=list(s.sign); bins=list(s.bins)
    cross=[f"{bins[i]}->{bins[i+1]}" for i in range(len(bins)-1) if signs[i]!=signs[i+1]]
    print(f"  logistic/none {bt}: signs={dict(zip(bins,signs))}")
