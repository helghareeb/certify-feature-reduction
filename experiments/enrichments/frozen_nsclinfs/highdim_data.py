"""High-dimensional (p >> 44) dataset loaders for the Tier-2 boundary arm (TASK-014).
Same contract as nsclinfs.data loaders: load_*(data_dir) -> (X: DataFrame, y: 0/1 np.ndarray, sensitive: Series).
Raw files live under C:\\research\\_staging\\highdim (local C:, NOT synced). Frozen nsclinfs.data is untouched;
...
HD = Path(r"C:\research\_staging\highdim")
