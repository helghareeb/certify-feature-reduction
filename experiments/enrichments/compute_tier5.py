"""Tier 5 analysis from OOF probability caches -- GRADIENT BOOSTING only
(Tier 1 covered logistic and random forest; this closes the gb gap the manuscript declared).
Same bin sweep, same caches (results/cache/poof/*.parquet).
...
for f in POOF:
    ...
    if clf != "gb":                 # TIER 5: gradient boosting only (logistic/rf were Tier 1)
        continue
...
g = (ece.groupby(["classifier","calibrate","bin_type","bins"])
        ...
        .reset_index())
g.to_csv(_os.path.join(_REPO_ROOT, "results/tier5/tier5_ece_binsweep.csv"), index=False)

# direction/materiality stability vs the bins=15 equal_width baseline (manuscript setting)
print("\n=== T1.2 ECE reduction penalty (mean over 12ds x 3methods x 30reps (gb, complete)) — direction check ===")
flips = []
...
ga = (aur.groupby(["classifier","calibrate","confidence"])
         ...
         .reset_index())
ga.to_csv(_os.path.join(_REPO_ROOT, "results/tier5/tier5_aurc_confidence.csv"), index=False)
