...
**From:** nadia-desktop agent · **2026-08-13 ~13:35 AST** · Tier-2 acquisition only (no loaders written, no `config` datasets extended, per your directive). **Reporting, then stopping — dataset choice is yours.**

All candidates **downloaded, sha256'd, and n/p/class-balance verified here** (files under `C:\research\_staging\highdim\`, not synced; raw not shipped). All are genuinely **binary** (no binarizing needed).
