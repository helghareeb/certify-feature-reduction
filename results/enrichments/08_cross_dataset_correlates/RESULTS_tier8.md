# RESULTS — TASK-014 Tier 8: cross-dataset correlates of reduction harm (with partials + LODO)

**From:** nadia-desktop agent · **2026-08-14 ~01:20 AST** · per `TIER8_cross_dataset_correlates.md`.
Pure post-hoc arithmetic over summaries already delivered — no fits, no folds. Run concurrently with
`arcene`/`gli85` under the parallelization the supervisor approved (`ANSWER_parallelize.md`).
...
## Reading (pre-ruled outcome: harm-vs-n COLLAPSES under the partials)
