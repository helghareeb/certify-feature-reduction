    ...
    for method in METHODS:
        for cname in CLFS:
            for rep in range(REPS):
                ...
                rows.append(dict(method=method, classifier=cname, rep=rep, auroc_full=af, auroc_red=ar, aurc_full=uf, aurc_red=ur))
    d = pd.DataFrame(rows); d["tag"]=tag
    d["auroc_penalty"]=d.auroc_full-d.auroc_red; d["aurc_penalty"]=d.aurc_red-d.aurc_full
    return d   # per-(rep,ranker,clf) rows -> dispersion
...
for ds in ["cleveland", "mammographic", "spectf"]:
    ...
    for dose in [0, 1, 2, 5, 10]:
        ...
        r["dataset"]=ds; r["dose"]=dose; r["p_full"]=X.shape[1]; r["k_aggr"]=k_aggr
        resA.append(r)
        print(f"  A {ds:12s} dose{dose:2d}x p={X.shape[1]:4d} k={k_aggr:3d} pen_mean={r.auroc_penalty.mean():+.3f} pen_sd={r.auroc_penalty.std():.3f}")
dfA = pd.concat(resA, ignore_index=True); dfA.to_csv(REPO / "results" / "p1_probe_injection_rows.csv", index=False)
# bootstrap CI on the per-cell mean penalty (over rep x ranker x clf)
def _ci(v, seed=20260626, B=2000):
    v=np.asarray(v,float); rng=np.random.RandomState(seed); bs=[v[rng.randint(0,len(v),len(v))].mean() for _ in range(B)]
    return round(float(np.percentile(bs,2.5)),4), round(float(np.percentile(bs,97.5)),4)
aggA=[]
for (ds,dose),g in dfA.groupby(["dataset","dose"]):
    lo,hi=_ci(g.auroc_penalty); aggA.append(dict(dataset=ds,dose=dose,p_full=int(g.p_full.iloc[0]),k_aggr=int(g.k_aggr.iloc[0]),
        auroc_penalty_mean=round(g.auroc_penalty.mean(),4),auroc_penalty_sd=round(g.auroc_penalty.std(),4),ci_lo=lo,ci_hi=hi,n=len(g)))
pd.DataFrame(aggA).to_csv(REPO / "results" / "p1_probe_injection.csv", index=False)
...
for keep in [100, 256, 1000, 5000, Xa.shape[1]]:
    ...
    k_aggr = 25   # FIXED absolute budget across all keep-sets (agent fix: decouple from k)
    r = evaluate(Xk, ya, k_aggr, f"arcene_keep{keep}")
    r["kept_features"]=keep; r["k_aggr"]=k_aggr
    resB.append(r)
    print(f"  B arcene keep{keep:5d} k={k_aggr}  pen_mean={r.auroc_penalty.mean():+.3f} pen_sd={r.auroc_penalty.std():.3f}")
dfB = pd.concat(resB, ignore_index=True); dfB.to_csv(REPO / "results" / "p1_arcene_reverse_rows.csv", index=False)
aggB=[]
for keep,g in dfB.groupby("kept_features"):
    lo,hi=_ci(g.auroc_penalty); aggB.append(dict(kept_features=int(keep),k_aggr=25,
        auroc_penalty_mean=round(g.auroc_penalty.mean(),4),auroc_penalty_sd=round(g.auroc_penalty.std(),4),ci_lo=lo,ci_hi=hi,n=len(g)))
pd.DataFrame(aggB).to_csv(REPO / "results" / "p1_arcene_reverse.csv", index=False)
