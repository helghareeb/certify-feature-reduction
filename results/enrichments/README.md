...
- `MANIFEST.sha256` — SHA-256 for every file in the folder. Some arms carry a second, sub-arm
  manifest as well. **Every `*.sha256` under `results/` verifies against the bytes released here, and
  CI checks all of them on every push** — 27 at the time of writing — so a stale hash cannot pass as
  a clean sweep.
