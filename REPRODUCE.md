...
```bash
python -m pytest tests/ -q               # expect: 27 passed
```

Those 27 tests are not decoration. They check the budget rule's rounding, that the analysis
...
**What each column means.** `paper/MANIFEST.toml` declares every committed column: either it is cited
...
#    exact invocations and measured runtimes: paper/LOCAL_EXECUTION_PLAYBOOK.md
...
against these files, and a third was corrected — `paper/AUDIT_REPORT.md` lists both what changed and
